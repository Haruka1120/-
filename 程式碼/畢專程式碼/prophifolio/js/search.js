    ...
    if (!context || http_stream) {
        ...
        if (async) {
            ...
            if (typeof notification === 'function') {
                if (0 && req.addEventListener) {} else {
...
$j(document).ready(function() {
    ...
    $j('#search_full888').focus(function() {
        searchbtn = $j('.gs02');
    });
    $j('#pkstock1,#pkstock2,#pkstock3').focus(function() {
        searchbtn = '';
    });
});

function sendphp () {
	var company = "<?php Print($output); ?>";
	$j.post('k_line.php', {number: company});
}


function GetPChomeCookie() {
    return;
}